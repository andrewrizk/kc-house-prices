...
```{r}
# Feature scaling
numericFeatures = sapply(select(house_dataset, -price), is.numeric)
...
formula<-as.formula(price ~  bedrooms + bathrooms + sqft_living + sqft_lot + sqft_above + floors + waterfront + view + condition + grade
...
lm_0<-stepAIC(lm(formula = formula2, 
                 data=training),
              trace=F)
summary(lm_0)
validation$pred_lm0 <- predict(lm_0, validation)

# Since the price was log transformed we will recompute the error metrics by reversing the log transformation
lm0_mae <- mae(real = expm1(validation$price), predicted = expm1(validation$pred_lm0))
lm0_mape <- mape(real = expm1(validation$price), predicted = expm1(validation$pred_lm0))
lm0_rmse <- rmse(real = expm1(validation$price), predicted = expm1(validation$pred_lm0))

paste("The mean absolute error for linear regression is ", lm0_mae, "and mape is", lm0_mape, "and rmse is ", lm0_rmse)
...
```{r}
training[,-which(names(training)== "price")]

matrix <- as.matrix(training[,-which(names(training)== "price")])
