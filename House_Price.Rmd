...
house_dataset <- house_dataset %>% mutate(long_bin = ifelse(long >= -2.168754 & long < -1, 'long_1',
...
house_dataset <- house_dataset %>% mutate(lat_bin = ifelse(lat >= -2.916866 & lat < -2.3, 'lat_1',
...
splits <- splitdf(train_data, seed = 11)
...
lm_model = lm(formula = price ~  bedrooms + bathrooms + sqft_living + sqft_lot + sqft_above + floors + waterfront + view + condition + grade + sqft_basement + zipcode + sqft_living15 + sqft_lot15 + yr_built + NewHouse + Age + long + lat,
           data = training)
...
# Plotting predictions vs real
ggplot(validation,aes(x=expm1(price),y=expm1(ridge_pred)))+geom_point()+geom_smooth(method = "lm") + theme_minimal() + labs(x="Real", y="Prediction") + ggtitle(paste("Ridge Regression -->",'RMSE:', format(round(ridge_rmse, 4)), " and MAPE -->", format(round(ridge_mape, 4)))) + labs(subtitle = paste(" and Mean Price Error:", format(round(ridge_mae,4)))) +
  scale_x_continuous(labels = function(x) format(x, scientific = FALSE)) +
  scale_y_continuous(labels = function(x) format(x, scientific = FALSE))
...
formula2 <- as.formula(price ~bedrooms + bathrooms + sqft_living + sqft_lot + sqft_above + floors + waterfront + view + condition + grade + sqft_basement + zipcode + sqft_living15 + sqft_lot15 + yr_built + NewHouse + Age + long + lat)
...
# Therefore, we will use a separate dataset where we convert zipcode into a numeric variable and then test our model
...
# Plotting predictions vs real
ggplot(rfvalidation,aes(x=expm1(price),y=expm1(rf0_pred)))+geom_point()+geom_smooth(method = "lm") + theme_minimal() + labs(x="Real", y="Prediction") +
...
```
After hyper-parameter optimization the coefficient of determination has only improved from 0.69 to 0.71.
...
paste("The mean absolute error for linear regression is ", xgb_mae, "and mape is", xgb_mape, "and rmse is ", xgb_rmse)
paste("XGB R-Squared:", r2_xgb)
...
##### Test Prediction
Now we will predict the test set using the best model. XGBoost gave the lowest mean absolute percentage error of 0.1172.
```{r}
